...
public class RetrofitHelper {
	 //----------- 싱글톤 객체 생성 시작 ----------
	  private static RetrofitHelper current = null;
	  ...
	  //----------- 싱글톤 객체 생성 끝 ----------
	  /**
	   *	Retrofit 객체를 생성하고 기본 설정들을 적용한 후 리턴 한다
	   *
	  ...
	  public Retrofit getRetrofit(String baseUrl) {
		  ...
		  // -> import okhttp3.logger.HttpLoggingInterceptor.level;
		  ...
		  //Builder를 통해 통신객체 생성 -> import okhttp3.Okhttp3.OkhttpClient
		  OkHttpClient httpClient = httpClientBuilder.build();
		  
		  //Retrofit을 생성하는 기능을 수행하는 객체를 정의 한다.
	  ...
	  /**
	   * 	Log4j 객체를 통해 로그를 기록하도록 하ㅡㄴ 메서드를 구현하는 클래스 정의
	  ...
	  /**
	   * 	통신절차를 가로채서 보조적인 처리를 수행하는 클래스 정의
	   * 	이 클래스가 통시 정보에 HTTP 헤더를 강제로 추가하는 기능을 수행한다.
	   * 	이 클래스의 객체를 통신 객체에 경결해야 한다.
	   */
	  // -> import okhttp3.Interceptor
	  public class MyInterCeptor implements Interceptor{
		  // -> import okhttp3.Response
		  public Response intercept(Chain chain) throws IOException{
			  Request originalRequest = chain.request();	//요정정보 객체 추출 -> import okhttp3.Request
			  Request.Builder builder = originalRequest.newBuilder(); //요청 정보 객체를 복제해서 새로운 객체 생성
			  builder.removeHeader("User-Agent");
			  
			  //임의의 웹 브라우저 정보로 설정 (ex:크롬)
			  builder.header("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.70 Safari/537.36");
